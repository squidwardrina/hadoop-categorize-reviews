...
public class GoodWordsMap extends
	...
	protected void map(
		...
		// Get the review id from the filename
		Path filePath = ((FileSplit) context.getInputSplit()).getPath();
		String revType = getRevType(filePath);

		// Get number of words
		String strReview = IOUtils.toString(file.getBytes(), ENCODING);
		Integer wordsCount = statsCalculator.getGoodWords(strReview);
		// Write the data to map-reduce context
		context.write(new Text(revType + DELIM + CheckType.goodWords.toString()),
